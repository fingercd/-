...
| 原生当前视频 smoke v3 | 进行中 | 13 条原生前向已通过；11 条等待；1 条因许可门禁 blocked |
...
当前 catalog 保留 25 个目标，但严格区分原生状态：13 条路线已经用各自代码和各自公开权重
通过当前视频 smoke，11 条路线仍为 `planned`，另有 1 条路线因许可门禁为 `blocked`，等待各自的官方 checkout、checkpoint、许可证
