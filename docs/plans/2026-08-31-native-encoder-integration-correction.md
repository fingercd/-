...
| `videochat_online` | [MCG-NJU/VideoChat-Online](https://github.com/MCG-NJU/VideoChat-Online)；[MCG-NJU/VideoChatOnline-4B](https://huggingface.co/MCG-NJU/VideoChatOnline-4B)，8.29 GB、MIT | 复现官方 ViT feature + Pyramid Memory Bank；实现真实 `visual_memory` state；当前公开代码明确未提供 KV-cache memory，不能注册为 decoder KV | Python 3.9/Transformers custom code；单卡 BF16/CPU 预检；必须连续两 chunk |
